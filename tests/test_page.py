...
class TestHomeBuddyPage:
    ...
    @title("Test contractor not found flow.")
    def test_contractor_not_found_flow(self, browser, reporter):
        """Test the HomeBuddy page."""
        hvac_page = HvacPage(browser)

        with reporter.step(browser=browser,
                           step_name="When user fill zip code and click on 'Get estimate' button"):
            hvac_page.fill_input("10001", "zipCode", "zipCode")
            hvac_page.click_on_button(HvacPage.button("Get estimate"))

        with reporter.step(browser=browser,
                           step_name="When user clicks on 'Repair' tile and 'Next' button"):
    ...
    @title("Test contractor not found flow with name {test_data[0]} and email {test_data[1]}.")
    def test_get_contractor_flow_wrong_name_email(self, browser, reporter, test_data):
        """Test the HomeBuddy page."""
        hvac_page = HvacPage(browser)
        ...
        with reporter.step(browser=browser,
                           step_name="When user clicks on 'Replacement' tile and 'Next' button"):
            hvac_page.click_on_button(HvacPage.tile_button("Replacement"))
            hvac_page.click_on_button(HvacPage.button("Next"))

        with reporter.step(browser=browser,
                           step_name="When user clicks on 'Water heater' tile and 'Next' button"):
            hvac_page.click_on_button(HvacPage.tile_button("Water heater"))
            hvac_page.click_on_button(HvacPage.button("Next"))

        with reporter.step(browser=browser,
                           step_name="When user clicks on 'Propane' tile and 'Next' button"):
            hvac_page.click_on_button(HvacPage.tile_button("Propane"))
            hvac_page.click_on_button(HvacPage.button("Next"))

        with reporter.step(browser=browser,
                           step_name="When user clicks on '5 to 10 years' tile and 'Next' button"):
        ...
        with reporter.step(browser=browser,
                           step_name="When user selects 'Not sure' checkbox and clicks 'Next' button"):
        ...
        with reporter.step(browser=browser,
                           step_name="When user fills incorrect 'Full Name' and 'Email' fields and clicks 'Next' button"
                           ):
        ...
        with reporter.step(browser=browser,
                           step_name="Then validation messages is displaying"):
